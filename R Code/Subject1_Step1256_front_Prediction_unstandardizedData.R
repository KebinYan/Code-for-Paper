...
acc_trainStand <- subset(acc_front_stand, acc_front_stand$timeElapsed > (acc_standTime - trainingSec/2) & acc_front_stand$timeElapsed < (acc_standTime + trainingSec/2))
acc_walkTime <- setDT(acc_front_walk)[, if(.N > 1) .SD[ceiling(.N/2):ceiling(.N/2)] else .SD]$timeElapsed
acc_trainWalk <- subset(acc_front_walk, acc_front_walk$timeElapsed > (acc_walkTime - trainingSec/2) & acc_front_walk$timeElapsed < (acc_walkTime + trainingSec/2))
acc_stairUpTime <- setDT(acc_front_stairUp)[, if(.N > 1) .SD[ceiling(.N/2):ceiling(.N/2)] else .SD]$timeElapsed
acc_trainStairUp<- subset(acc_front_stairUp, acc_front_stairUp$timeElapsed > (acc_stairUpTime - trainingSec/2) & acc_front_stairUp$timeElapsed < (acc_stairUpTime + trainingSec/2))
acc_stairDownTime <- setDT(acc_front_stairDown)[, if(.N > 1) .SD[ceiling(.N/2):ceiling(.N/2)] else .SD]$timeElapsed
acc_trainStairDown <- subset(acc_front_stairDown, acc_front_stairDown$timeElapsed > (acc_stairDownTime - trainingSec/2) & acc_front_stairDown$timeElapsed < (acc_stairDownTime + trainingSec/2))
...
gyro_trainStand <- subset(gyro_front_stand, gyro_front_stand$timeElapsed > (gyro_standTime - trainingSec/2) & gyro_front_stand$timeElapsed < (gyro_standTime + trainingSec/2))
gyro_walkTime <- setDT(gyro_front_walk)[, if(.N > 1) .SD[ceiling(.N/2):ceiling(.N/2)] else .SD]$timeElapsed
gyro_trainWalk <- subset(gyro_front_walk, gyro_front_walk$timeElapsed > (gyro_walkTime - trainingSec/2) & gyro_front_walk$timeElapsed < (gyro_walkTime + trainingSec/2))
gyro_stairUpTime <- setDT(gyro_front_stairUp)[, if(.N > 1) .SD[ceiling(.N/2):ceiling(.N/2)] else .SD]$timeElapsed
gyro_trainStairUp<- subset(gyro_front_stairUp, gyro_front_stairUp$timeElapsed > (gyro_stairUpTime - trainingSec/2) & gyro_front_stairUp$timeElapsed < (gyro_stairUpTime + trainingSec/2))
gyro_stairDownTime <- setDT(gyro_front_stairDown)[, if(.N > 1) .SD[ceiling(.N/2):ceiling(.N/2)] else .SD]$timeElapsed
gyro_trainStairDown <- subset(gyro_front_stairDown, gyro_front_stairDown$timeElapsed > (gyro_stairDownTime - trainingSec/2) & gyro_front_stairDown$timeElapsed < (gyro_stairDownTime + trainingSec/2))
...
#combined_chairStand2 <- combineData(acc_front_chairStand2, gyro_front_chairStand2)
#write.csv(combined_walk, "~/Desktop/data/subject1/training/combined/walk_front.csv")
#write.csv(combined_stand, "~/Desktop/data/subject1/training/combined/stand_front.csv")
#write.csv(combined_stairUp, "~/Desktop/data/subject1/training/combined/stairUp_front.csv")
#write.csv(combined_stairDown, "~/Desktop/data/subject1/training/combined/stairDown_front.csv")
#write.csv(combined_chairStand1, "~/Desktop/data/subject1/training/combined/chairStand1_front.csv")
#write.csv(combined_chairStand2, "~/Desktop/data/subject1/training/combined/chairStand2_front.csv")
...
trainStand <- subset(stand_front, stand_front$acc.timestamp > (standTime - trainingSec/2) & stand_front$acc.timestamp < (standTime + trainingSec/2))
walkTime <- setDT(walk_front)[, if(.N > 1) .SD[ceiling(.N/2):ceiling(.N/2)] else .SD]$acc.timestamp
trainWalk <- subset(walk_front, walk_front$acc.timestamp > (walkTime - trainingSec/2) & walk_front$acc.timestamp < (walkTime + trainingSec/2))
stairUpTime <- setDT(stairUp_front)[, if(.N > 1) .SD[ceiling(.N/2):ceiling(.N/2)] else .SD]$acc.timestamp
trainStairUp<- subset(stairUp_front, stairUp_front$acc.timestamp > (stairUpTime - trainingSec/2) & stairUp_front$acc.timestamp < (stairUpTime + trainingSec/2))
stairDownTime <- setDT(stairDown_front)[, if(.N > 1) .SD[ceiling(.N/2):ceiling(.N/2)] else .SD]$acc.timestamp
trainStairDown <- subset(stairDown_front, stairDown_front$acc.timestamp > (stairDownTime - trainingSec/2) & stairDown_front$acc.timestamp < (stairDownTime + trainingSec/2))
...
# write.csv(test_front_step1, "~/Desktop/data/subject1/test/Step1/combined_front.csv")
# test_front_step2 <- combineData(test_acc_front_step2, test_gyro_front_step2)
# write.csv(test_front_step2, "~/Desktop/data/subject1/test/Step2/combined_front.csv")
#test_front_step5 <- combineData(test_acc_front_step5, test_gyro_front_step5)
#write.csv(test_front_step5, "~/Desktop/data/subject1/test/Step5/combined_front.csv")
#test_front_step6_stairDown <- combineData(test_acc_front_step6_stairDown, test_gyro_front_step6_stairDown)
#write.csv(test_front_step6_stairDown, "~/Desktop/data/subject1/test/Step6/stairDown/combined_front.csv")
#test_front_step6_stairUp <- combineData(test_acc_front_step6_stairUp, test_gyro_front_step6_stairUp)
#write.csv(test_front_step6_stairUp, "~/Desktop/data/subject1/test/Step6/stairUp/combined_front.csv")

# the data sets were created by the above codes
test_front_step1 <- read.csv("~/Desktop/data/subject1/test/Step1/combined_front.csv")
test_front_step2 <- read.csv("~/Desktop/data/subject1/test/Step2/combined_front.csv")
test_front_step5 <- read.csv("~/Desktop/data/subject1/test/Step5/combined_front.csv")
test_front_step6_stairDown <- read.csv("~/Desktop/data/subject1/test/Step6/stairDown/combined_front.csv")
test_front_step6_stairUp <- read.csv("~/Desktop/data/subject1/test/Step6/stairUp/combined_front.csv")
...
#write.csv(combined_result_step1, "~/Desktop/data/subject1/test/Prediction/Step1/4s Training Set/front_pred.csv")
...
#write.csv(combined_result_step2, "~/Desktop/data/subject1/test/Prediction/Step2/4s Training Set/front_pred.csv")
...
write.csv(combined_result_step5, "~/Desktop/data/subject1/test/Prediction/Step5/5s Training Set/front_pred.csv")
...
write.csv(combined_result_step6_stairUp, "~/Desktop/data/subject1/test/Prediction/Step6/5s Training Set/stairUp/front_pred.csv")
...
write.csv(combined_result_step6_stairDown, "~/Desktop/data/subject1/test/Prediction/Step6/5s Training Set/stairDown/front_pred.csv")
...
#write.csv(acc_result_step1, "~/Desktop/data/subject1/test/Prediction/Step1/4s Training Set/acc_front_pred.csv")
...
#write.csv(acc_result_step2, "~/Desktop/data/subject1/test/Prediction/Step2/5s Training Set/acc_front_pred.csv")
...
write.csv(acc_result_step5, "~/Desktop/data/subject1/test/Prediction/Step5/5s Training Set/acc_front_pred.csv")
...
write.csv(acc_result_step6_stairDown, "~/Desktop/data/subject1/test/Prediction/Step6/5s Training Set/stairDown/acc_front_pred.csv")
...
write.csv(acc_result_step6_stairUp, "~/Desktop/data/subject1/test/Prediction/Step6/5s Training Set/stairUp/acc_front_pred.csv")
...
write.csv(gyro_result_step1, "~/Desktop/data/subject1/test/Prediction/Step1/5s Training Set/gyro_front_pred.csv")
...
write.csv(gyro_result_step2, "~/Desktop/data/subject1/test/Prediction/Step2/5s Training Set/gyro_front_pred.csv")
...
write.csv(gyro_result_step5, "~/Desktop/data/subject1/test/Prediction/Step5/5s Training Set/gyro_front_pred.csv")
gyro_result_step5 <- movelet_Bai2012_singleSensor_modified(test_gyro_front_step5, gyro_axes_step5, gyro_training, frequency, moveletLength, distOption, trainingActivities, useMag)
write.csv(gyro_result_step5, "~/Desktop/data/subject1/test/Prediction/Step5/5s Training Set/gyro_front_pred.csv")
...
write.csv(gyro_result_step6_stairDown, "~/Desktop/data/subject1/test/Prediction/Step6/5s Training Set/stairDown/gyro_front_pred.csv")
...
write.csv(gyro_result_step6_stairUp, "~/Desktop/data/subject1/test/Prediction/Step6/5s Training Set/stairUp/gyro_front_pred.csv")
...
activityCols <- c("green", "black", "red", "blue","yellow","purple","cyan")
...
plotActivityPrediction(combined_result_step1, xRange, activityList, activityCols)
plotActivityPrediction(combined_result_step2, xRange, activityList, activityCols)
plotActivityPrediction(combined_result_step5, xRange, activityList, activityCols)
plotActivityPrediction(combined_result_step6_stairUp, xRange, activityList, activityCols)
plotActivityPrediction(combined_result_step6_stairDown, xRange, activityList, activityCols)

plotActivityPrediction(acc_result_step1, xRange, activityList, activityCols)
plotActivityPrediction(acc_result_step2, xRange, activityList, activityCols)
plotActivityPrediction(acc_result_step5, xRange, activityList, activityCols)
plotActivityPrediction(acc_result_step6_stairDown, xRange, activityList, activityCols)
plotActivityPrediction(acc_result_step6_stairUp, xRange, activityList, activityCols)

plotActivityPrediction(gyro_result_step1, xRange, activityList, activityCols)
plotActivityPrediction(gyro_result_step2, xRange, activityList, activityCols)
plotActivityPrediction(gyro_result_step5, xRange, activityList, activityCols)
plotActivityPrediction(gyro_result_step6_stairDown, xRange, activityList, activityCols)
plotActivityPrediction(gyro_result_step6_stairUp, xRange, activityList, activityCols)
